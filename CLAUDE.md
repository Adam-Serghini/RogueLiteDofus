...
Implemented beyond the original specs (added by playtest iteration): the **6-element system** (terre/feu/eau/air/wakfu/stasis with Chance/Wakfu/Stasis stats), **7 classes** (Iop/Cra/Eniripsa/Sadida/Sram/Feca/Ecaflip), a **recruitment system** (start with 2, recruit/heal at tavernes, team of 4 max), and **equipment** (`ITEMS`/`PANOPLIES` in `data.ts`; **7 slots** incl. `arme`/weapon; **3 zone-bound panoplies** — Aventurier in Incarnam, Paysan in Champs d'Astrub, Bouftou in Tainéla, mapped per zone in `BUTIN_ZONE`; item stats are **rolled at drop** from `[min,max]` ranges (`Item.rolls` → an `ItemInstance` with concrete `stats` via `rollItem`), so two copies differ; duplicates allowed; **per-run** loot — `RunState.inventaire` + `PersoState.equipement` hold `ItemInstance` objects, lost on death; **Prospection** boosts drop rate). Equipment stats/PV/resistances fold into the combatant via `bonusEquipement`/`pvMaxPerso` in `run.ts`.

Also: **Archimonstres & Dofus Ocre** — any enemy can rarely spawn as a boosted "Archi" variant (`ARCHI` in `data.ts`); defeating it captures the species once (persistent `Meta.archis`), tracked in a per-zone **Bestiaire** (`showBestiaire`). The **Dofus Ocre** is no longer a boss drop — it unlocks in tiers every 50 captures (`OCRE_PALIERS`), granting escalating team passives (PA + damage) via `bonusEquipe(meta)` (combines Dofus + Ocre). Not reachable with the current ~16 monsters by design (framework for a bigger bestiary, ~286).
...
Each tranche is deliberately narrow; the spec's validation criteria define "done". Current build: **7 classes** (Iop/Cra/Eniripsa/Sadida/Sram/Feca/Ecaflip), **6 elements**, in-run levels/points, recruitment (start at 2, taverne recruit/heal, max 4), **equipment** (3 zone-bound panoplies, 7 slots, rolled-stat per-run loot), **3 sequential zones** (Incarnam → Champs d'Astrub → Tainéla) each a generated board ending in a boss+miniboss donjon that drops a Dofus, and a 31-relic collection (only Dofus Pourpre has a gameplay effect so far). Everything in the "out of scope" list above is **deferred** — resist pulling it in. **Balance is unverified**: the new classes' damage and the start-at-2 difficulty are tuned by formula, not playtest — values in `data.ts`/`progression.ts` are meant to be adjusted. Crit is now a multiplicative bonus (affects players AND monsters).
